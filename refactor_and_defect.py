...
defect_det = T5ForConditionalGeneration.from_pretrained('Salesforce/codet5-base').to(device)
refiner = T5ForConditionalGeneration.from_pretrained('Salesforce/codet5-base').to(device)


defect_det.load_state_dict(torch.load('defect_codet5_base.bin',
                                     map_location=device))
refiner.load_state_dict(torch.load('refine_medium_codet5_base.bin',
                                     map_location=device))
